...
public class GridServiceInjectionSpringResourceTest extends GridCommonAbstractTest {
    ...
    /** */
    private static final String springCfgFileTemplate =
        "src/test/java/org/apache/ignite/spring/injection/spring-resource.tmpl.xml";

    /** */
    private static String springConfigFileOutTmplName = "spring-resource.xml-";
    ...
    @Override protected void afterTest() throws Exception {
        ...
        for (int i = 0; i < NODES; ++i)
            Files.deleteIfExists(FS.getPath(springConfigFileOutTmplName + i));
    ...
    private void startNodes() throws Exception {
        AbstractApplicationContext ctxSpring = new FileSystemXmlApplicationContext(springConfigFileOutTmplName + 0);
        ...
        for (int i = 1; i < NODES; ++i)
            new FileSystemXmlApplicationContext(springConfigFileOutTmplName + i);
    ...
    private void generateConfigXmls(int nodes) throws IOException {
        ...
        try (BufferedReader br = Files.newBufferedReader(FS.getPath(springCfgFileTemplate), UTF_8)) {
            while (true) {
        ...
        for (int i = 0; i < nodes; ++i) {
            try (BufferedWriter bw = Files.newBufferedWriter(FS.getPath(springConfigFileOutTmplName + i), UTF_8)) {
